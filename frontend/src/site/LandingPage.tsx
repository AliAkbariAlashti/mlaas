...
import { ArrowLeft, ArrowRight, Check, Menu, Play, X } from "lucide-react";
...
function Header({ openAuth }: { openAuth: () => void }) {
  ...
  const nav = [
    ["#services", localize(lang, "Product", "محصول")],
    ["#workflow", localize(lang, "Solutions", "راهکارها")],
    ["#case-study", localize(lang, "Customers", "مشتریان")],
    ["#weblog", localize(lang, "Resources", "منابع")],
    ["#company", localize(lang, "Company", "شرکت")]
  ];
  return <header className="marketing-header"><div className="marketing-container header-row">
    <Brand />
    <nav className={menu ? "nav-open" : ""}>{nav.map(([href, label]) => <a key={href} href={href} onClick={() => setMenu(false)}>{label}</a>)}</nav>
...
function AuthDialog({ close }: { close: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      else { const result = await api.verifyOtp(phone, otp); api.tokens.set(result.access_token, result.refresh_token); await refreshUser(); close(); navigate("/app"); }
  ...
  return <div className="dialog-backdrop" onMouseDown={close}><div className="auth-dialog" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true">
    ...
    <h2>{localize(lang, step === "phone" ? "Sign in to your workspace" : "Enter the verification code", step === "phone" ? "ورود به فضای کاری" : "کد تأیید را وارد کنید")}</h2>
    <p>{localize(lang, step === "phone" ? "Use your mobile number to continue. New customers receive three analysis credits." : `A six-digit code was sent to ${phone}.`, step === "phone" ? "برای ادامه شماره موبایل را وارد کنید. مشتریان جدید سه اعتبار تحلیل دریافت می‌کنند." : `کد شش‌رقمی به ${phone} ارسال شد.`)}</p>
    <form onSubmit={submit}><label>{localize(lang, step === "phone" ? "Mobile number" : "Verification code", step === "phone" ? "شماره موبایل" : "کد تأیید")}</label><input autoFocus value={step === "phone" ? phone : otp} onChange={event => step === "phone" ? setPhone(event.target.value) : setOtp(event.target.value)} placeholder={step === "phone" ? "09123456789" : "123456"}/>{error && <span className="form-error">{error}</span>}<button className="button button-solid full" disabled={busy}>{busy ? localize(lang, "Please wait…", "لطفاً صبر کنید…") : localize(lang, step === "phone" ? "Continue" : "Verify and continue", step === "phone" ? "ادامه" : "تأیید و ادامه")}</button></form>
    {step === "otp" && <button className="inline-action" onClick={() => setStep("phone")}>{localize(lang, "Change mobile number", "تغییر شماره موبایل")}</button>}
  </div></div>;
}
...
function Weblog() {
  ...
  const articles = posts.length ? posts : [
    { slug:"commerce-analytics-foundation", title_en:"From raw data to decisions: building a commerce analytics foundation", title_fa:"از داده خام تا تصمیم: ساخت پایه تحلیل تجارت", excerpt_en:"A practical framework for connecting transaction data to consistent business action.", excerpt_fa:"چارچوبی عملی برای اتصال داده تراکنش به اقدام منسجم کسب‌وکار.", published_at:"2026-07-12", category_en:"Analytics",category_fa:"تحلیل" },
    { slug:"retention-segments", title_en:"Which customer segments deserve attention first?", title_fa:"کدام بخش‌های مشتری ابتدا به توجه نیاز دارند؟", excerpt_en:"Use recency and repeat behavior to choose a useful retention priority.", excerpt_fa:"با تازگی و تکرار خرید، اولویت مفید حفظ مشتری را انتخاب کنید.", published_at:"2026-07-05", category_en:"Retention",category_fa:"حفظ مشتری" },
    { slug:"basket-rules", title_en:"Reading basket rules without overinterpreting them", title_fa:"چگونه قوانین سبد خرید را درست تفسیر کنیم", excerpt_en:"A clear guide to support, confidence, lift, and responsible merchandising.", excerpt_fa:"راهنمای روشن پشتیبانی، اطمینان، لیفت و فروش مسئولانه.", published_at:"2026-06-28", category_en:"Merchandising",category_fa:"فروش" }
  ];
  const featured = articles[0];
  return <section className="weblog-section" id="weblog"><div className="marketing-container"><div className="weblog-heading"><h2>{localize(lang, "Ideas for better commerce decisions.", "ایده‌هایی برای تصمیم‌های بهتر در تجارت.")}</h2><a href="#weblog-list">{localize(lang, "Explore all articles", "مشاهده همه نوشته‌ها")}<ArrowRight/></a></div><div className="featured-article"><div className="editorial-art"><div className="editorial-bars">{[30,48,72,56].map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div><svg viewBox="0 0 400 220"><polyline points="10,190 110,140 180,160 260,70 390,35"/></svg><span className="editorial-circle"/></div><article><span>{localize(lang, featured.category_en??"Analytics", featured.category_fa??"تحلیل")}</span><h3>{localize(lang, featured.title_en, featured.title_fa)}</h3><p>{localize(lang, featured.excerpt_en, featured.excerpt_fa)}</p><time>{new Date(featured.published_at).toLocaleDateString(lang === "fa" ? "fa-IR" : "en-US", {year:"numeric",month:"long",day:"numeric"})}</time><a href={`#article-${featured.slug}`}>{localize(lang, "Read article", "مطالعه نوشته")}<ArrowRight/></a></article></div><div className="article-list" id="weblog-list">{articles.slice(1).map((article:any) => <article id={`article-${article.slug}`} key={article.slug}><div className="article-thumb"><span/><i/><i/><i/></div><span>{localize(lang, article.category_en??"Insight", article.category_fa??"بینش")}</span><h3>{localize(lang, article.title_en, article.title_fa)}</h3><p>{localize(lang, article.excerpt_en, article.excerpt_fa)}</p><time>{new Date(article.published_at).toLocaleDateString(lang === "fa" ? "fa-IR" : "en-US")}</time><ArrowRight/></article>)}</div></div></section>;
}
...
function ContactFooter({ openAuth }: { openAuth: () => void }) {
  ...
  ].map(group => <div key={group[0] as string}><strong>{localize(lang,group[0] as string,group[1] as string)}</strong>{(group[2] as string[][]).map(item=><a href="#" key={item[0]}>{localize(lang,item[0],item[1])}</a>)}</div>)}</div><div className="marketing-container footer-bottom"><span>© 2026 InsightFlow. {localize(lang,"All rights reserved.","تمام حقوق محفوظ است.")}</span><ThemeLanguageControls/></div></footer>
...
export function LandingPage() {
  const { lang } = useApp(); const [auth, setAuth] = useState(false); const [services, setServices] = useState<Service[]>(fallbackServices);
